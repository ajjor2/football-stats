...
function createPlayerStatCardElement(stats) {
    ...
    if (stats.pastMatchesDetails && stats.pastMatchesDetails.length > 0) {
        const matchesHtml = stats.pastMatchesDetails.slice(0, 10).map(match => { // Limit to 10 matches
            let matchDisplay;
    ...
    // MUOKATTU OSIO: Muiden joukkueiden ottelut (yksityiskohtainen)
    ...
    if (stats.otherTeamsDetailedMatches && Object.keys(stats.otherTeamsDetailedMatches).length > 0) {
        ...
        // Järjestä joukkueiden osiot joukkueen nimen (displayKey) mukaan aakkosjärjestykseen
        ...
        for (const teamDisplayKey of sortedOtherTeamKeys) {
            ...
            if (teamMatches.length > 0) {
                ...
                teamMatches.slice(0, 10).forEach(match => { // Rajoitetaan näytettävien otteluiden määrää / muu joukkue
                    let matchDisplay;
                    if (match.status === "Fixture") {
                        matchDisplay = `${match.date}: ${match.playerTeamNameInPastMatch} vs ${match.opponentName} (Tuleva)`;
                    } else {
