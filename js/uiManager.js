...
function createPlayerStatCardElement(stats) {
    ...
    // Modified: Combine games, goals, and last played date per team
    let gamesAndGoalsByTeamDisplayContent;
    ...
    if (stats.gamesPlayedThisYear > 0 && gamesBreakdownKeys.length > 0) {
        let detailsHtml = Object.entries(stats.gamesByTeamThisYear)
            .map(([displayKey, teamGames]) => {
                const teamGoals = stats.goalsByTeamThisYear && stats.goalsByTeamThisYear[displayKey] ? stats.goalsByTeamThisYear[displayKey] : 0;
                const lastPlayedDate = stats.lastPlayedDateByTeam && stats.lastPlayedDateByTeam[displayKey] ? stats.lastPlayedDateByTeam[displayKey] : 'N/A';
                // Simple date formatting, assuming YYYY-MM-DD
                const formattedDate = lastPlayedDate !== 'N/A' ? lastPlayedDate.split('-').reverse().join('.') : 'N/A';
                return `${displayKey}: ${teamGames} ottelua, ${teamGoals} maalia (Viim. pelattu: ${formattedDate})`;
            })
            .join('<br>');
        
        if (gamesBreakdownKeys.length > 1 || stats.gamesPlayedThisYear > (stats.gamesByTeamThisYear[gamesBreakdownKeys[0]] || 0) ) { // Show summary if multiple teams or if total games > games for the single listed team
             detailsHtml += `<br><b>Yhteensä: ${stats.gamesPlayedThisYear} ottelua, ${stats.goalsThisYear} maalia</b>`;
        }
        gamesAndGoalsByTeamDisplayContent = detailsHtml;
    } else {
        // Display total games and goals if no per-team breakdown is available but totals exist
        gamesAndGoalsByTeamDisplayContent = `${stats.gamesPlayedThisYear} ottelua, ${stats.goalsThisYear} maalia`;
    }
    ...
    if (stats.pastMatchesDetails && stats.pastMatchesDetails.length > 0) {
        const matchesHtml = stats.pastMatchesDetails.slice(0, 10).map(match => {
            let matchDisplay;
            // Ensure date is formatted, assuming it comes as YYYY-MM-DD
            const displayDate = match.date ? match.date.split('-').reverse().join('.') : 'Pvm tuntematon';
            if (match.status === "Fixture") {
                matchDisplay = `${displayDate}: ${match.playerTeamNameInPastMatch} vs ${match.opponentName} (Tuleva)`;
            } else {
                const indicatorClass = match.resultIndicator || 'draw';
                matchDisplay = `<span class="result-indicator ${indicatorClass}"></span>
                                ${displayDate}: ${match.playerTeamNameInPastMatch} vs ${match.opponentName} (${match.playerTeamScore}-${match.opponentScore})`;
            }
    ...
    card.innerHTML = `
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
            ...
            ${createStatItemHtml(`Ottelut & Maalit Joukkueittain (${config.CURRENT_YEAR})`, gamesAndGoalsByTeamDisplayContent)}
...
function displayMatchInfo(match, groupDataForMatchInfo, lineupGoalsA = 0, lineupGoalsB = 0, container) {
    ...
    if (groupDataForMatchInfo && groupDataForMatchInfo.matches) {
        ...
        if (previousEncounters.length > 0) {
            ...
            previousEncounters.forEach(enc => {
                const encDate = enc.date ? enc.date.split('-').reverse().join('.') : 'Pvm tuntematon';
                headToHeadHtml += `<li>${encDate}: ${enc.team_A_name} ${enc.fs_A !== undefined ? enc.fs_A : '-'} - ${enc.fs_B !== undefined ? enc.fs_B : '-'} ${enc.team_B_name}</li>`;
            });
        ...
        const getRefereeGamesList = (targetTeamId, targetTeamName) => {
            ...
            if (games.length > 0) {
                ...
                games.forEach(pastGame => {
                    ...
                    const pastGameDate = pastGame.date ? pastGame.date.split('-').reverse().join('.') : 'Pvm tuntematon';
                    listHtml += `<li class="referee-past-game-item"><span class="result-indicator ${resultIndicatorClass}"></span> ${pastGameDate}: vs ${opponentName} (${teamScore !== undefined ? teamScore : '-'}-${opponentScore !== undefined ? opponentScore : '-'})</li>`;
                });
                listHtml += `</ul></div>`;
                return listHtml;
            }
    ...
    const matchDate = match.date ? match.date.split('-').reverse().join('.') : 'N/A';
    if (container) {
        container.innerHTML = `
            ...
            <p class="text-sm text-gray-500 text-center mt-3">Päivämäärä: ${matchDate}</p>
...
function displayGroupInfoAndStandings(group, currentMatchTeamAId, currentMatchTeamBId, container) {
    if (!container) return; // Guard clause if container is not available
    ...
    sortedTeams.forEach(team => {
        ...
        tableHtml += `
            <tr class="${isCurrentTeam ? 'bg-blue-100' : ''}">
...
async function processAndDisplayPlayerStats(playersInMatch, matchDetails, groupDataForInfo, playerStatsContainerGlobal, matchInfoContainerGlobal) {
    ...
    allPlayerStats.sort((a, b) => {
        ...
        // Sort by captain status (captain first)
        const isCaptainA = a.isCaptainInMatch ? 0 : 1;
        const isCaptainB = b.isCaptainInMatch ? 0 : 1;
        if (isCaptainA !== isCaptainB) return isCaptainA - isCaptainB;

        const numA = parseInt(a.shirtNumber);
        const numB = parseInt(b.shirtNumber);
        if (!isNaN(numA) && !isNaN(numB) && numA !== numB) return numA - numB;
        if (!isNaN(numA)) return -1; // Players with numbers first
        if (!isNaN(numB)) return 1;  // Players with numbers first
...
async function displayPlayersNotInLineup(teamDetails, matchLineupPlayerIds, teamDisplayName, originalMatchDetails, container) {
    ...
    if (playersNotInMatch.length > 0) {
        ...
        } else if (playersNotInMatch.length > 0) { // If there were players but fetching stats failed for all
            const noPlayersMsg = document.createElement('p');
            noPlayersMsg.className = 'text-gray-600';
            noPlayersMsg.textContent = 'Ei muita pelaajia näytettäväksi tälle joukkueelle (tilastojen haku saattoi epäonnistua kaikille).';
            sectionDiv.appendChild(noPlayersMsg);
        }
        // Only append the section if there's something to show (header + cards or header + message)
