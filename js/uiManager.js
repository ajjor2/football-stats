...
function createPlayerStatCardElement(stats) {
    ...
    if (stats.gamesPlayedThisYear > 0 && gamesBreakdownKeys.length > 0) {
        ...
        if (gamesBreakdownKeys.length > 1 || stats.gamesPlayedThisYear > (stats.gamesByTeamThisYear[gamesBreakdownKeys[0]] || 0) ) { // Show summary if multiple teams or if total games > games for the single listed team (handles multiple leagues for same team name case better)
