...
function createPlayerStatCardElement(stats) {
    ...
    if (stats.goalsThisYear > 0 && stats.goalsByTeamThisYear && Object.keys(stats.goalsByTeamThisYear).length > 0) {
        const contextTeamName = stats.pastMatchesDetails.length > 0 ? stats.pastMatchesDetails[0].playerTeamNameInPastMatch : null;
        ...
        if (showBreakdown) {
            goalsDisplayContent = Object.entries(stats.goalsByTeamThisYear)
                ...
                .join('<br>');
            if (Object.keys(stats.goalsByTeamThisYear).length > 1 || (Object.keys(stats.goalsByTeamThisYear).length === 1 && stats.goalsThisYear > stats.goalsByTeamThisYear[Object.keys(stats.goalsByTeamThisYear)[0]])) {
                 goalsDisplayContent += `<br><b>Yhteensä: ${stats.goalsThisYear}</b>`;
            }
        } else if (Object.keys(stats.goalsByTeamThisYear).length === 1) {
    ...
    if (stats.gamesPlayedThisYear > 0 && stats.gamesByTeamThisYear && Object.keys(stats.gamesByTeamThisYear).length > 0) {
        const contextTeamName = stats.pastMatchesDetails.length > 0 ? stats.pastMatchesDetails[0].playerTeamNameInPastMatch : null;
        let showBreakdown = Object.keys(stats.gamesByTeamThisYear).length > 1;
         if (!showBreakdown && Object.keys(stats.gamesByTeamThisYear).length === 1) {
            const singleTeam = Object.keys(stats.gamesByTeamThisYear)[0];
        ...
        if (showBreakdown) {
            gamesPlayedDisplayContent = Object.entries(stats.gamesByTeamThisYear)
                ...
                .join('<br>');
            if (Object.keys(stats.gamesByTeamThisYear).length > 1 || (Object.keys(stats.gamesByTeamThisYear).length === 1 && stats.gamesPlayedThisYear > stats.gamesByTeamThisYear[Object.keys(stats.gamesByTeamThisYear)[0]])) {
                 gamesPlayedDisplayContent += `<br><b>Yhteensä: ${stats.gamesPlayedThisYear}</b>`;
            }
        } else if (Object.keys(stats.gamesByTeamThisYear).length === 1) {
    ...
    let pastMatchesForContextTeamHtml = '';
    if (stats.pastMatchesDetails && stats.pastMatchesDetails.length > 0) {
        const contextTeamName = stats.pastMatchesDetails[0].playerTeamNameInPastMatch;
        ...
        pastMatchesForContextTeamHtml = `<div class="mt-4 pt-4 border-t border-gray-200 col-span-1 sm:col-span-2">
                                <h4 class="text-md font-semibold text-gray-700 mb-2">Pelatut ottelut joukkueelle ${contextTeamName} (${config.CURRENT_YEAR}):</h4>
                                <ul class="list-none pl-0 space-y-1">${matchesHtml}</ul></div>`;
    }

    let gamesForOtherTeamsHtml = '';
    if (stats.gamesForOtherTeamsThisYear && stats.gamesForOtherTeamsThisYear.length > 0) {
        const matchesHtml = stats.gamesForOtherTeamsThisYear.slice(0, 10).map(match => {
            let matchDisplay;
             const competitionContext = `${match.competition_name || ''}${match.category_name && match.category_name !== match.competition_name ? ` (${match.category_name})` : ''}${match.group_name ? ` - ${match.group_name}` : ''}`;
            if (match.status === "Fixture") {
                matchDisplay = `${match.date}: ${match.playerTeamNameInPastMatch} vs ${match.opponentName} (Tuleva)`;
            } else {
                const indicatorClass = match.resultIndicator || 'draw';
            ...
            return `<li class="past-match-item">${matchDisplay} <span class="text-xs text-gray-500">(${competitionContext || 'Sarja tuntematon'})</span></li>`;
        }).join('');
        gamesForOtherTeamsHtml = `<div class="mt-4 pt-4 border-t border-gray-200 col-span-1 sm:col-span-2">
                                <h4 class="text-md font-semibold text-gray-700 mb-2">Pelit muille joukkueille (${config.CURRENT_YEAR}):</h4>
                                <ul class="list-none pl-0 space-y-1">${matchesHtml}</ul></div>`;
    }
    ...
    card.innerHTML = `
        ...
        ${pastMatchesForContextTeamHtml}
        ${gamesForOtherTeamsHtml}
    `;
    return card;
}
