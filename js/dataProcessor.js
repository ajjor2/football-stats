...
function processPlayerMatchHistory(matches, currentSeasonId, previousSeasonId, teamNameForContext) {
    const stats = {
        ...
        goalsByTeamThisYear: {}, gamesByTeamThisYear: {}, // These will be modified
    ...
    matches.forEach(pastMatch => {
        ...
        if (isCurrentSeason) {
            ...
            // UUSI: Hae sarjan nimi, käytä competition_name ensisijaisesti, sitten category_name
            const leagueName = pastMatch.competition_name || pastMatch.category_name || 'Sarja tuntematon';
            const displayKey = `${teamNameForGame} (${leagueName})`;

            // Käytä displayKeytä gamesByTeamThisYear-objektissa
            ...
            if (playerGoals > 0) {
                // Käytä displayKeytä goalsByTeamThisYear-objektissa
